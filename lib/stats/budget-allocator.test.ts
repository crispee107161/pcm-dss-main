...
// projected_inquiries for each ad set should equal its allocated spend
// exactly — makes the allocation math independently verifiable by hand.
...
describe('computeBudgetAllocation', () => {
  ...
  it('allocates budget proportional to Laplace-smoothed efficiency across two ad sets', async () => {
    ...
    vi.mocked(prisma.ad.findMany).mockResolvedValue([
      { ad_set_name: 'A', amount_spent: 500, total_messaging_contacts: 50, reach: 250 },
      { ad_set_name: 'A', amount_spent: 500, total_messaging_contacts: 50, reach: 250 },
      { ad_set_name: 'B', amount_spent: 500, total_messaging_contacts: 5, reach: 250 },
      { ad_set_name: 'B', amount_spent: 500, total_messaging_contacts: 5, reach: 250 },
    ] as any)

    // Set A: spend=1000, messaging=100 -> cpiEstimate=10, efficiency=(101)/(1000+10)=0.1
    // Set B: spend=1000, messaging=10  -> cpiEstimate=100, efficiency=(11)/(1000+100)=0.01
    ...
    // With coef_reach=0 and coef_amount_spent=1, projected messaging
    // conversations must equal allocated spend exactly (residual_std_error=0
    ...
    // Total projected messaging conversations should equal the total
    // budget, since every peso allocated maps 1:1 under this model.
  ...
  it('caps allocation at the top 8 most efficient ad sets', async () => {
    ...
    // 10 ad sets, spend=100 each, messaging = i+1 for i in 0..9.
    // efficiency simplifies to messaging/100, so higher count -> higher rank.
    const ads = Array.from({ length: 10 }, (_, i) => ({
      ad_set_name: `Set-${i}`,
      amount_spent: 100,
      total_messaging_contacts: i + 1,
      reach: 50,
    }))
    ...
    expect(names).not.toContain('Set-0') // messaging=1, lowest efficiency
    expect(names).not.toContain('Set-1') // messaging=2, second lowest
    expect(names[0]).toBe('Set-9') // messaging=10, highest efficiency, sorted first
  })
  ...
  it('throws when no ad set has any messaging conversations recorded', async () => {
    vi.mocked(prisma.regressionModel.findFirst).mockResolvedValue(linearSpendModel as any)
    vi.mocked(prisma.ad.findMany).mockResolvedValue([
      { ad_set_name: 'A', amount_spent: 500, total_messaging_contacts: 0, reach: 250 },
    ] as any)
    await expect(computeBudgetAllocation(1000)).rejects.toThrow('No ad sets with messaging conversation data found')
  })
})
