...
describe('expandAndAggregate', () => {
  it('spreads a multi-day ad evenly across each day it ran', () => {
    ...
    expect(result.get('2026-01-01')).toEqual({ reach: 100, messaging: 10, amount_spent: 50 })
    expect(result.get('2026-01-02')).toEqual({ reach: 100, messaging: 10, amount_spent: 50 })
  })
  ...
  it('treats null reach/messaging as zero rather than throwing', () => {
    const ads = [
      {
        reporting_starts: new Date('2026-01-01'),
        reporting_ends: new Date('2026-01-01'),
    ...
    expect(expandAndAggregate(ads).get('2026-01-01')).toEqual({ reach: 0, messaging: 0, amount_spent: 10 })
  })
})
...
describe('computeLaggedCorrelations', () => {
  ...
  it('identifies the correct lag when messaging conversations are an exact function of reach N days earlier', async () => {
    ...
    const ads = reach.map((r, d) => {
      ...
      const messaging = d < TRUE_LAG ? 0 : reach[d - TRUE_LAG]
      return {
        reporting_starts: date,
        reporting_ends: date,
        reach: r,
        total_messaging_contacts: messaging,
        amount_spent: 1,
      }
    })
