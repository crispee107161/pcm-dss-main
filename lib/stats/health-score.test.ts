...
describe('computeHealthScores', () => {
  ...
  it('scores the best-performing ad in the set as Excellent (100)', () => {
    // CPA 10 (best of the two priced ads), rate 0.01 (best), reach 1000 (top of range).
    const ads = [
      ad({ id: 1, amount_spent: 100, purchases: 10, reach: 1000 }),
      ad({ id: 2, amount_spent: 200, purchases: 5, reach: 1000 }),
    ]

    const [scoredA] = computeHealthScores(ads)

    expect(scoredA.cpa).toBe(10)
    expect(scoredA.purchase_rate).toBe(0.01)
  ...
  it('scores a mid-performing ad using the 0.50/0.35/0.15 weighting exactly', () => {
    ...
    // cpa_score=0 (worst CPA), rate_score=50 (half of best rate), reach_score=100 (tied for top reach)
    // 0*0.50 + 50*0.35 + 100*0.15 = 17.5 + 15 = 32.5 -> rounds to 33
    expect(scoredB.score).toBe(33)
    expect(scoredB.grade).toBe('Poor')
  })

  it('gives an ad with zero purchases a low score without throwing on null cpa/rate', () => {
    const ads = [
      ad({ id: 1, amount_spent: 100, purchases: 10, reach: 1000 }),
      ad({ id: 2, amount_spent: 200, purchases: 5, reach: 1000 }),
      ad({ id: 3, amount_spent: 50, purchases: 0, reach: 500 }),
    ]

    const scoredC = computeHealthScores(ads)[2]

    expect(scoredC.cpa).toBeNull()
    expect(scoredC.purchase_rate).toBeNull()
  ...
  it('never lets a single outlier CPA compress every other score to the same band', () => {
    // The 95th-percentile cap means one extreme ad shouldn't flatten the distribution.
    const ads = [
      ad({ id: 1, amount_spent: 100, purchases: 10, reach: 1000 }),
      ad({ id: 2, amount_spent: 10000, purchases: 1, reach: 1000 }), // extreme outlier CPA
    ]
  ...
  it('clamps a score at 100 for an ad above the 95th-percentile reach cap, rather than exceeding it', () => {
    ...
    const ads = Array.from({ length: 21 }, (_, i) =>
      ad({ id: i, amount_spent: 100, purchases: 0, reach: (i + 1) * 100 })
    )
