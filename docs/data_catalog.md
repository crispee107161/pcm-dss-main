...
This file is **four separate blocks** stacked in one CSV, each with its own mini-header, and must be parsed as four distinct sections rather than one table:

1. `Age & gender` — a small age-bracket × gender percentage matrix (`18-24`, `25-34`, … `65+` rows).
2. `Top cities` — one row of city names, one row of percentages.
3. `Top countries` — same shape as top cities.
4. **`Follows`** — an embedded 365-row daily series, **identical in shape and content to `Follows (1).csv`**.

**Do not ingest block 4 as a second `follows` source** — it duplicates `Follows (1).csv` exactly and would double-count if both were upserted naively (though the natural key on `date` would make a second upsert a no-op update, not a duplicate row, the values must match or a silent overwrite occurs). Recommend treating `Audience.csv` as demographic-only and ignoring its trailing Follows block, or explicitly asserting it matches `Follows (1).csv` before discarding it.

Blocks 1–3 are demographic breakdowns with no FR currently depending on them — same treatment as §3.3.
