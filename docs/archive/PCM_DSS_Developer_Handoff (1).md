...
**Method.** Rank all messaging-optimised ads by cost per inquiry, split into quartiles, and compare.

Verified output from the current 12 months (n = 187 ads):
...
**The headline number:** Q4 spent ₱38,527 for 983 inquiries. At Q1's rate of ₱12.50, that same money would have produced **≈3,086 inquiries — about 2,100 more, for zero additional spend.**
...
Verified output — **this contradicts the standard "ad fatigue" advice:**
...
CPI **improves** with age (₱14.96 → ₱12.57). Frequency correlates *negatively* with CPI (Spearman ρ = −0.241, p < 0.001, n = 482), and frequency is low overall (median 1.55, p90 2.14, max 3.22).
...
**Screen requirements:** watch-through per video post, median by content category and by post type, and a scatter of watch-through vs. engagement rate. Guard the division: exclude `Duration (sec)` ≤ 0.
...
Verified: **389,577 visits → 11,386 new follows = 2.92% follow rate** over 12 months. Monthly follows range 652 → 1,641.
