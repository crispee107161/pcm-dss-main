...
export default async function SalesPrintReportPage() {
  ...
  const [allAds, latestModel, categories, dailyMetrics, lagData] = await Promise.all([
    prisma.ad.findMany({
      select: {
        ad_name: true,
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="max-w-3xl mx-auto p-8 md:p-12 space-y-8">
        ...
        <ReportHeader
          title="PC Merchandise — Sales Performance Report"
        ...
        <ReportSection title="Key Insights">
          <ul className="space-y-2 text-sm text-neutral-700">
            ...
            {totalImpressions > 0 && totalLinkClicks > 0 && (
              <li className="flex gap-2">
                <span className="text-neutral-400 mt-0.5">—</span>
                Overall link click-through rate: <strong className="text-neutral-900">{((totalLinkClicks / totalImpressions) * 100).toFixed(2)}%</strong> ({totalLinkClicks.toLocaleString()} clicks / {totalImpressions.toLocaleString()} impressions)
              </li>
            )}
