...
export default async function SalesReportPage() {
  ...
  const [allAds, latestModel, categories] = await Promise.all([
    prisma.ad.findMany({
      select: {
  ...
  const cpa = adsWithPurchases.length > 0 ? totalSpend / totalPurchases : null
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div id="report-content" className="space-y-6 print:space-y-8">
        ...
        <div className="bg-gradient-to-r from-red-700 to-red-500 rounded-2xl p-8 text-white">
          <h1 className="text-2xl font-bold">PC Merchandise — Sales Performance Report</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50">
                {['Period', 'Ads Run', 'Total Spend', 'Purchases', 'Reach', 'ROAS'].map(h => (
        ...
        {topCampaigns.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 print-no-break">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50">
                  {['#', 'Ad Name', 'Spend', 'Purchases', 'CPA'].map(h => (
                    <th key={h} className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider px-4 py-3">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {topCampaigns.map((ad, i) => (
                  <tr key={i} className="border-t border-slate-100">
                    <td className="px-4 py-3 text-slate-500 font-medium">{i + 1}</td>
                    <td className="px-4 py-3 font-medium text-slate-800 max-w-xs truncate">{ad.ad_name}</td>
                    <td className="px-4 py-3 text-slate-700">{formatPHP(ad.amount_spent)}</td>
                    <td className="px-4 py-3 text-green-700 font-semibold">{ad.purchases}</td>
                    <td className="px-4 py-3 text-slate-600">
                      {ad.purchases ? formatPHP(ad.amount_spent / ad.purchases) : '—'}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-2">
                  For every ₱1 increase in ad spend, approximately {(latestModel.coefficient * 1000).toFixed(3)} additional purchases are projected.
                </p>
              </div>
        ...
        <div className="bg-slate-900 rounded-2xl p-6 print-no-break">
          ...
          <ul className="space-y-2 text-sm text-zinc-300">
            ...
            {allAds.length === 0 && (
              <li className="flex gap-2">
                <span className="text-red-400 mt-0.5">•</span>
                No ad data available yet. The Marketing Manager must upload Facebook Ads CSV data.
              </li>
            )}
          </ul>
        </div>
