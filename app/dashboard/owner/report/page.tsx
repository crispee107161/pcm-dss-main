      ...
      <div id="report-content" className="space-y-6 print:space-y-8">
              ...
              <tbody>
                {monthlyData.map(row => (
                  <tr key={row.period} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-slate-600 hidden sm:table-cell">{row.spend > 0 ? (row.purchases / row.spend * 1000).toFixed(4) : '—'}</td>
                  </tr>
                ))}
                <tr className="border-t-2 border-slate-200 bg-slate-50 font-semibold">
                  ...
                  <td className="px-4 py-3 text-slate-600 hidden sm:table-cell">{totalSpend > 0 ? (totalPurchases / totalSpend * 1000).toFixed(4) : '—'}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        {dailyMetrics.length >= 7 && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 print-no-break">
            ...
            <p className="text-xs text-slate-400 mb-4">MA baseline: {viewsForecast.lastMA.toLocaleString()} views/day</p>
          ...
          <ul className="space-y-2 text-sm text-zinc-300">
            ...
            {lagData.best_r !== null && lagData.best_metric && (
              <li className="flex gap-2">
                ...
                Purchases peak <strong>{lagData.best_lag} day{lagData.best_lag !== 1 ? 's' : ''}</strong> after ad metrics (strongest: {lagData.best_metric}, r = {lagData.best_r.toFixed(4)}).
              </li>
            )}
            {latestModel && (
              <li className="flex gap-2">
                <span className="text-red-400 mt-0.5">•</span>
                {isMLR ? 'MLR' : 'Regression'} model explains <strong>{(latestModel.r_squared * 100).toFixed(1)}%</strong> of purchase variance
                {isMLR && latestModel.residual_std_error != null ? ` with ±${(latestModel.residual_std_error * 1.2816).toFixed(1)}-purchase 80% PI.` : '.'}
              </li>
            )}
            {allPosts.length > 0 && (
              <li className="flex gap-2"><span className="text-red-400 mt-0.5">•</span>Organic posts avg engagement: <strong>{avgEngagement.toFixed(2)}%</strong> ({allPosts.length} posts).</li>
            )}
            {totalImpressions > 0 && totalLinkClicks > 0 && (
              <li className="flex gap-2"><span className="text-red-400 mt-0.5">•</span>Link CTR: <strong>{((totalLinkClicks / totalImpressions) * 100).toFixed(2)}%</strong></li>
            )}
            {viewsForecast.forecast.length > 0 && (
              <li className="flex gap-2"><span className="text-red-400 mt-0.5">•</span>7-day page views forecast baseline: <strong>{viewsForecast.lastMA.toLocaleString()}/day</strong>.</li>
            )}
          </ul>
        </div>
