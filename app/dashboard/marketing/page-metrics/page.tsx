...
export default async function PageMetricsPage() {
  ...
  // --- 7-day MA forecast for page views (FR-22) ---
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      {!hasAnyData && postCount === 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center mb-8">
          ...
          <h2 className="text-slate-700 font-semibold mb-2">No page data uploaded yet</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-w-xl mx-auto text-left mb-6">
            {[
              ...
              <div key={file} className="bg-slate-50 rounded-lg px-3 py-2">
                <p className="text-xs font-mono text-slate-700">{file}</p>
        ...
        <section className="mb-10">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <p className="text-xs text-slate-400 mt-3">
              Note: The forecast projects the trailing 7-day moving average forward as a flat baseline. It does not account for seasonality or campaign changes.
            </p>
          </div>
        </section>
      )}
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {genderData.length > 0 ? (
            ...
            <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-8 text-center">
              <p className="text-slate-400 text-sm mb-2">No gender data. Upload FollowerGender.csv.</p>
          ...
          {territoryData.length > 0 ? (
            ...
            <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-8 text-center">
              <p className="text-slate-400 text-sm mb-2">No territory data. Upload FollowerTopTerritories.csv.</p>
              <Link href="/dashboard/marketing/upload" className="text-red-600 hover:underline text-xs">Upload data →</Link>
            </div>
          )}
        </div>
