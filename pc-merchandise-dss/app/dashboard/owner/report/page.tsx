...
export default async function OwnerReportPage() {
  ...
  const [allAds, allPosts, latestModel, categories] = await Promise.all([
    prisma.ad.findMany({
      select: {
        reporting_starts: true, amount_spent: true, purchases: true,
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div id="report-content" className="space-y-6 print:space-y-8">
        ...
        {allPosts.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 print-no-break">
            <h2 className="text-lg font-bold text-slate-800 mb-4 pb-2 border-b border-slate-100">Organic Post Performance</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
        ...
        {(Object.keys(adCatCounts).length > 0 || Object.keys(postCatCounts).length > 0) && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 print-no-break">
            <h2 className="text-lg font-bold text-slate-800 mb-4 pb-2 border-b border-slate-100">Content Category Distribution</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
        ...
        <div className="bg-slate-900 rounded-2xl p-6 print-no-break">
          <h2 className="text-lg font-bold text-slate-900 mb-3">Key Insights</h2>
