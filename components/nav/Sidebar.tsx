...
export default function Sidebar({ navItems, email, roleLabel, roleBadgeClass, children }: SidebarProps) {
    ...
    if (item.section && item.section !== currentSection) {
      ...
      rendered.push(
        <p
          ...
          className={`px-3 pt-4 pb-1 text-xs font-semibold text-zinc-500 uppercase tracking-widest ${collapsed ? 'md:hidden' : ''}`}
        >
          {item.section}
        </p>
      )
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 bg-black text-white flex flex-col print:hidden transition-[transform,width] duration-200
          md:translate-x-0
      ...
      >
        ...
        {/* Nav */}
        <nav className={`flex-1 overflow-y-auto py-3 space-y-0.5 ${collapsed ? 'md:px-2 px-3' : 'px-3'}`}>
          {rendered}
        </nav>
        ...
        <div className="border-t border-zinc-800 relative flex-shrink-0" ref={dropdownRef}>
          ...
          {dropdownOpen && (
            <div
              className={`animate-fade-slide-up absolute z-10 bg-zinc-900 border border-zinc-700 rounded-xl overflow-hidden card-shadow-floating ${
                collapsed
          ...
          <button
            ...
            className={`w-full flex items-center transition-[background-color] hover:bg-zinc-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-red-500 ${
          ...
          >
            ...
            <svg
              className={`w-3.5 h-3.5 text-zinc-500 flex-shrink-0 transition-transform duration-200 ${dropdownOpen ? 'rotate-180' : ''} ${collapsed ? 'md:hidden' : ''}`}
              fill="none" viewBox="0 0 24 24" stroke="currentColor"
            >
      ...
      {/* Main content */}
      <div
        className={`flex-1 min-h-screen print:ml-0 flex flex-col overflow-x-hidden transition-[margin] duration-200 ${
